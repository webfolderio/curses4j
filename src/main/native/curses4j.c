...
jint curses4j_copywin(JNIEnv *env, jobject that, jlong src_w, jlong dst_w, jint src_tr,
            jint src_tc, jint dst_tr, jint dst_tc, jint dst_br,
            jint dst_bc, jint _overlay) {
  WINDOW* src = *(WINDOW **) &src_w;
  WINDOW* dst = *(WINDOW **) &dst_w;
  return copywin(src, dst, src_tr, src_tc, dst_tr, dst_tc, dst_br, dst_bc, _overlay);
}

jint curses4j_dupwin(JNIEnv *env, jobject that, jlong peer) {
  WINDOW* win = *(WINDOW **) &peer;
  return dupwin(win);
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
  ...
  JNINativeMethod methods[] = {
    ...
    { "curses4j_dupwin", "(J)I", (void*) curses4j_dupwin }
  };
